...
class SolutionsRepository {
  ...
  async findSolution(email: string, name: string): Promise<Solution | null> {
    ...
    const result = await db.collection<SolutionDTO>(this.collection).findOne({ owner: email, name })

    if (result) {
      const { _id, createdAt, ...solution } = result

      return { id: _id.toString(), ...(solution as Solution) }
    }
